  ...
  if(WriteAddr<STM32_FLASH_BASE||WriteAddr%2)return;	//非法地址
	...
	if(addrx<0X1FFF0000)			//只有主存储区,才需要执行擦除操作!!
	{
		while(addrx<endaddr)		//扫清一切障碍.(对非FFFFFFFF的地方,先擦除)
		{
			...
			{   
				offaddr=addrx-STM32_FLASH_BASE;		//实际偏移地址.
	      secpos=offaddr/STM_SECTOR_SIZE;			//扇区地址  0~127 for STM32F103RBT6
				status=STMFLASH_ErasePage(secpos*STM_SECTOR_SIZE+STM32_FLASH_BASE);;
...
void STMFLASH_WriteNoErase(u32 WriteAddr,u16 *pBuffer,u32 NumToWrite)	
{ 
	u8 status=0;
	u32 addrx=0;
	u32 endaddr=0;	
  if(WriteAddr<STM32_FLASH_BASE||WriteAddr%2)return;	//非法地址
	...
	if(status==0)
	{
		...
		{
			if(STMFLASH_WriteHalfWord(WriteAddr,*pBuffer))//写入数据
			{ 
				break;	//写入异常
			}
			WriteAddr+=2;
